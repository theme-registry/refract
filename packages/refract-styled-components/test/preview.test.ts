/**
 * §20 for the styled-components adapter — its output is not something a browser loads, so `preview.html`
...
const makeTmp = (): string => {
  const dir = mkdtempSync(join(tmpdir(), "sc-preview-"));
  tmpDirs.push(dir);
  return dir;
};
...
describe("styled-components adapter preview.html", () => {
  it("renders token plates and explains why recipes can't render live", async () => {
    const outDir = makeTmp();
    await emitTheme({ raw: RAW, adapter: createStyledComponentsAdapter(), outDir, preview: true });
    ...
    expect(html).toContain("emits JavaScript modules that render through React + styled-components");
